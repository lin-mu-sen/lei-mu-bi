...
- [Development guidelines](#development-guidelines)
    - [General Litecoin Core](#general-woodcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play woodcoins" on the test network, if you
...
([`valgrind.supp`](https://github.com/woodcoin-project/woodcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_woodcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_woodcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/woodcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_woodcoin.coverage/index.html`.
```
...
To profile a running woodcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep woodcoind` -- sleep 60
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `woodcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `woodcoin-cli` and the GUI debug console use this table to determine how to
