Litecoin Core version 0.13.2 is now available from:
  ...
  <https://download.woodcoin.org/woodcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a woodcoin
...
This allows running woodcoind without having to do any manual configuration.
...
`woodcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /woodcoinrpc>
        ...
        # optional bypass woodcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< woodcoinrpc:<password>
    </Location>
...
Restarting woodcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in woodcoin.conf
...
woodcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/woodcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `woodcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `woodcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of woodcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of woodcoins being spent is exactly
...
- `woodcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `woodcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
